...
const updateUsuario = async (nombre, apellido, mail, password, id) => {
  const client = new Client(config);
  await client.connect();
  ...
  try {
      const result = await client.query('UPDATE users SET nombre = $1, apellido = $2, mail = $3, password = $4 WHERE id = $5 RETURNING *',
